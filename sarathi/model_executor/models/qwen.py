...
class QWenLMHeadModel(nn.Module):
    ...
    def load_weights(
        ...
        for name, loaded_weight in hf_model_weights_iterator(
            ...
            # Non-first PP stage shouldn't load embedding.
            if pp_rank != 0 and "wte" in name:
                continue

            # Non-last PP stage shouldn't load lm_head / ln_f.
            ...
            # -----------------------------
            # FIX: Support both "transformer.h.<id>." and "model.h.<id>."
            # HF Qwen often uses "transformer.h.*" keys.
            # PP stages keep only local layer indices (0..layers_per_stage-1),
            # so we must remap global <id> to local <id-first_layer_id>.
            # -----------------------------
            m = re.search(r"(?:^|\.)(transformer|model)\.h\.(\d+)\.", name)
            if m:
                layer_id = int(m.group(2))
                if layer_id < first_layer_id or layer_id > last_layer_id:
                    continue
                new_layer_id = layer_id - first_layer_id
                # Replace ONLY the first occurrence of ".h.<layer_id>."
                name = re.sub(
                    rf"\.h\.{layer_id}\.",
                    rf".h.{new_layer_id}.",
                    name,
            ...
            # Tensor-parallel slicing for fused QKV.
            if "c_attn" in name:
            ...
            # MLP gate_up weight packing (w2, w1) -> gate_up_proj (2 * intermediate)
            ...
            # Now load remaining params normally.
            if name not in state_dict:
                # Fail fast with a clearer error message than KeyError.
                # This makes debugging naming mismatches easier.
                raise KeyError(
                    f"Parameter name not found in local state_dict: {name}. "
                    f"(pp_rank={pp_rank}, tp_rank={tp_rank}, "
                    f"pp_world={pp_world_size}, tp_world={tp_world_size})"
                )
